...
for (const [scene,tag] of [['kitaterao-window-3d','day'],['kitaterao-window-3d-night','night']]) {
  ...
  await p.evaluate(()=>window.__town3dFlyPose(0,9,-26,0,-0.05)); await p.waitForTimeout(500)
  await p.evaluate(()=>window.__town3dLand(true)); await p.waitForTimeout(4200)
  await p.screenshot({ path:`scripts/_shots/motes-${tag}.png` })
