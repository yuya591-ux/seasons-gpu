...
const spots=[['a',0,-2],['b',-20,-44],['c',40,-30],['d',-30,-10],['e',12,-50],['f',-14,6]]
for(const [tag,x,z] of spots){
  await p.evaluate(([x,z])=>window.__town3dFlyPose(x,7,z,0,-0.04),[x,z]).catch(()=>{}); await p.waitForTimeout(500)
  await p.evaluate(()=>window.__town3dLand && window.__town3dLand(true)).catch(()=>{}); await p.waitForTimeout(2400)
  await p.screenshot({ path:`g6-${tag}.png` }); console.log(tag)
