...
for (const e of evs) {
  const a = await draw()
  for (let i = 0; i < 12; i++) { await p.evaluate(n => window.__town3dEvent && window.__town3dEvent(n), e).catch(() => {}); await p.waitForTimeout(40) }
  await p.waitForTimeout(1500) // イベントが終わって後始末する時間
  const c = await draw()
  console.log(e.padEnd(15), 'dTex', (c.texMem - a.texMem), 'dGeo', (c.geoMem - a.geoMem), 'dCalls', (c.calls - a.calls), 'dProgs', (c.progs - a.progs))
}
