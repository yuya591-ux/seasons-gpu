...
const p = await b.newPage({ viewport: { width: 800, height: 600 }, deviceScaleFactor: 1.7 })
...
for (const [scene,tag] of [['kitaterao-window-3d-spring','spring'],['kitaterao-window-3d-autumn','autumn']]) {
  ...
  // 公園(16,-27)に降りる
  await p.evaluate(()=>window.__town3dFlyPose(16,7,-20,0,-0.06)); await p.waitForTimeout(500)
  await p.evaluate(()=>window.__town3dLand(true)); await p.waitForTimeout(4500)
  await p.screenshot({ path:`scripts/_shots/flowers-${tag}.png` })
}
