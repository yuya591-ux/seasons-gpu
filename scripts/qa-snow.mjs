...
for(const s of ['kitaterao-window-3d-snow','shishigaya-window-3d-snow','kitaterao-window-3d-snow-night']){
  await p.evaluate(x=>window.__applyScene(x), s).catch(()=>{}); await p.waitForTimeout(2800)
  await p.screenshot({ path:`snow-${s}.png` }); console.log('shot '+s)
}
