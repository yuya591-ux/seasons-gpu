...
> 眺めて、整う。四季と天気の、窓辺のひととき。

![窓辺 — 夕暮れの窓から見る街](public/og.jpg)

四季 × 天気 × 時間帯 × 環境音を、ただ**眺めて・聴いて・整う**アンビエント・アプリ。
高台の窓辺から街・海・山・谷戸を見渡し、雨や雪や夕焼け・夜の灯りの中で心をしずめる。
描画は画像や動画に頼らず、すべて **WebGL（GLSL シェーダー）で「現象」を計算生成**している。

**▶ 体験する: https://yuya591-ux.github.io/seasons/**

## できること

- **情景を選ぶ**: 四季×天気×時間帯の組み合わせを 15 以上収録（角部屋／下町／海辺／山あい／屋上／鶴見・獅子ヶ谷の谷戸…）。
  時刻に合わせて空の色と光がゆっくり移ろう。「いま」ボタンで今の季節・時刻の窓辺へ。
...
- **描画**: 生 WebGL のフルスクリーン・シェーダー（GLSL ES 1.00）。後処理に FXAA（輪郭の平滑化）。
  雨粒の屈折・水面・雲影・霧・粒子・見下ろす街のレイマーチ等を、画像を使わず計算で出す。
...
- **原則**: 外部 API 非依存・課金なし・個人キー非依存（「自分がいなくなっても動く」）。

### なぜ Three.js ではなく軽量シェーダー構成か

主役は「画面いっぱいの一枚絵をフラグメントシェーダーで描く」こと。生 WebGL なら依存最小・起動が軽く、
GLSL を直接握って質感を作り込める。後処理 AA のためのオフスクリーン 1 パスのみ自前で持つ。
...
npm run preview  # ビルド結果のプレビュー（:4790 で起動して検証スクリプトが叩く）
```
...
src/engine/renderer.js      WebGL レンダラ（情景ごとのシェーダー・自動品質・FXAA）
src/shaders/                情景ごとの GLSL（cornerRoom/windowTown/windowSea/windowMountains/shishigaya/…）
