...
const FLY = {
  ...
  bound: { x: 580, xMax: 580, zMin: -590, zMax: 42, yMax: 122, yFloor: 4.5 }, // x=西の大正の港町(x≈-490)／xMax=東の城下町(x≈470)／zMin=北の戦国の山城(z≈-486)まで飛べる（3拠点を別方角・遠距離に）
...
export async function mountTown3d(parent, opts = {}) {
  ...
  const SEA = { coast: 64, shore: 82, level: -10, floor: -13.5, westCoast: -150, westShore: -172 } // 東岸(x>64)に加え西岸(x<-150)も海へ落とす＝西の海の向こうに大正の港町
  ...
  const heightAt = (x, z) => {
    ...
    if (edd < EDO.r) { const t = Math.max(0, (edd - 68) / (EDO.r - 68)); let base = 5.5 - t * t * 16
      const undul = Math.min(1, Math.max(0, (edd - 18) / 28)) // 城の周り(edd<18)は平ら、外周ほど丘の起伏
      base += (Math.sin((x - EDO.x) * 0.062) * 1.4 + Math.cos((z - EDO.z) * 0.054) * 1.1 + Math.sin((x + z) * 0.04) * 0.7) * undul
      const sd = edoStream(x, z); base -= Math.min(1, Math.max(0, (5.2 - sd) / 2.6)) * 2.4 // 蛇行する小川を平底に掘り込む（中央2.6は平らな河床、縁で立ち上がる）
      h = Math.max(h, base) } // 起伏する平場＋小川（のっぺりした台地を脱す）
  ...
  if (kind === 'yato') {
    ...
    // ── 海・港（街の東の縁が湾へ下る）。空を映す広い水面＋防波堤＋灯台＋小舟＝飛んで海まで行ける。──
    {
      ...
      const wtex = new THREE.CanvasTexture(wc); wtex.wrapS = wtex.wrapT = THREE.RepeatWrapping; wtex.repeat.set(74, 46); seaTex = wtex
      const seaGeo = new THREE.PlaneGeometry(1220, 760); seaGeo.rotateX(-Math.PI / 2)
      ...
      seaMesh.position.set(15, SEA.level, -255); seaMesh.receiveShadow = true; town.add(seaMesh) // x≈-595..625・z≈-635..125 を広く覆う（西=大正の港町(x-490)／東=城下町(x470)／北=戦国の山城(z-486)への渡りの海）
      // ── 海の向こうの城下町（江戸）。海を渡るとやがて霞(fog)の向こうに天守が現れる＝M1の“reveal”。──
      {
        ...
        const tRoof = toon(season === 'winter' ? (isNight ? 0x8a9098 : 0xb8bcc0) : (isNight ? 0x47403a : 0x6f5f4d)), tWall = mottleMat(season === 'winter' ? 0xd9d3c5 : 0xcbc0a9, 170, 0.12, [1.2, 1.2]) // 壁に漆喰/木のムラ＝ベタ塗りを脱す水彩の手触り
        ...
        const wallBMat = mottleMat(season === 'winter' ? 0xeae6dc : 0xe2ddd0, 170, 0.1, [1.2, 1.2]), wall3Mat = mottleMat(season === 'winter' ? 0xb8b0a2 : 0x9a8a70, 170, 0.14, [1.3, 1.3]) // 土蔵の漆喰/板壁の木目
      ...
      // ── 北の海の果ての戦国の山城（時代の異なる第2の目的地。Edoとは遠く海で隔て共視界に入れない）──
      {
        ...
        // 山裾の前山・岩礁（一つの円錐でなく、海から小さな尾根や岩が連なる＝単調を脱す自然な山裾）
        for (let k = 0; k < 9; k++) { const a = (k / 9) * 6.2832 + 0.3, rr = SENGOKU.r * (0.92 + R() * 0.5), hx = sx + Math.cos(a) * rr, hz = sz + Math.sin(a) * rr, hh = 6 + R() * 12
          const hill = new THREE.Mesh(new THREE.ConeGeometry(7 + R() * 5, hh, 6 + (k % 2)), toon(season === 'winter' ? 0xc4c8c4 : season === 'autumn' ? 0x7e7044 : 0x5e6a48)); hill.position.set(hx, SEA.level - 1 + hh / 2, hz); hill.rotation.y = R() * 3; hill.castShadow = true; hill.receiveShadow = true; town.add(hill); town.add(addOutline(hill)) // 前山
          for (let j = 0; j < 2; j++) { const rk = new THREE.Mesh(new THREE.IcosahedronGeometry(1.4 + R() * 1.6, 0), toon(0x76706a)); rk.position.set(hx + (R() - 0.5) * 10, SEA.level + 0.3 + R(), hz + (R() - 0.5) * 10); rk.rotation.set(R() * 3, R() * 3, R() * 3); rk.scale.y = 0.6; town.add(rk) } } // 海の岩礁
        ...
        for (let i = 0; i < st.length; i++) {
          const w = st[i][0], h = st[i][1], rw = (w + 1.5) * 0.72, rh = 1.9
          const wall = new THREE.Mesh(new THREE.BoxGeometry(w, h, w), sWall); wall.position.set(sx, yb + h / 2, sz); wall.castShadow = true; town.add(wall); town.add(addOutline(wall))
          for (const a2 of [0, Math.PI / 2, Math.PI, -Math.PI / 2]) { const win = new THREE.Mesh(new THREE.BoxGeometry(0.7, h * 0.34, 0.06), toon(isNight ? 0xe8b86a : 0x20242a)); win.position.set(sx + Math.sin(a2) * (w / 2 + 0.04), yb + h * 0.55, sz + Math.cos(a2) * (w / 2 + 0.04)); win.rotation.y = a2; town.add(win) }
          const roof = new THREE.Mesh(new THREE.ConeGeometry(rw, rh, 4), sRoof); roof.rotation.y = Math.PI / 4; roof.position.set(sx, yb + h + rh / 2 - 0.15, sz); roof.castShadow = true; town.add(roof); town.add(addOutline(roof))
          yb += h + rh - 0.9
        }
        for (const sgn of [-1, 1]) { const sh = new THREE.Mesh(new THREE.ConeGeometry(0.3, 0.95, 6), toon(0xb89038)); sh.position.set(sx + sgn * 1.1, yb + 0.15, sz); sh.rotation.z = sgn * -0.3; town.add(sh) } // 鯱
        for (const a of [0.8, 3.7]) { const tx = sx + Math.cos(a) * 9, tz = sz + Math.sin(a) * 9, ty = coneY(9); const tur = new THREE.Mesh(new THREE.BoxGeometry(2.4, 3.0, 2.4), sWall); tur.position.set(tx, ty + 1.5, tz); tur.castShadow = true; town.add(tur); town.add(addOutline(tur)); const tr = new THREE.Mesh(new THREE.ConeGeometry(2.3, 1.4, 4), sRoof); tr.rotation.y = Math.PI / 4; tr.position.set(tx, ty + 3.7, tz); town.add(tr) } // 隅櫓×2
        ...
        const samWall = mottleMat(season === 'winter' ? 0xc8c2b6 : 0xab9c84, 150, 0.13, [1.3, 1.3]), samWall2 = mottleMat(season === 'winter' ? 0xdcd8ce : 0x8a7a62, 150, 0.13, [1.3, 1.3]), samRoof = mottleMat(season === 'winter' ? (isNight ? 0x7a828a : 0xa8b0b6) : (isNight ? 0x2e2a24 : 0x46402f), 150, 0.12, [1.8, 1.8]), samRoof2 = mottleMat(isNight ? 0x383229 : 0x5a4e3a, 150, 0.12, [1.8, 1.8]) // 侍屋敷の板壁の木目＋瓦の濃淡
      ...
      // ── 西の海の向こうの大正の港町（赤レンガ倉庫・時計塔・看板建築・桟橋・蒸気船・ガス灯）。海を渡るとやがて霞から現れる ──
      {
        ...
        const tWallA = mottleMat(season === 'winter' ? 0xd8d2c4 : 0xcfc3ab, 170, 0.12, [1.4, 1.4]), tRoofT = mottleMat(isNight ? 0x4a4038 : 0x6e5a48, 150, 0.12, [1.8, 1.8])
        const twA = [], twC = [], trT = [], tlit = [], tmM = new THREE.Matrix4()
