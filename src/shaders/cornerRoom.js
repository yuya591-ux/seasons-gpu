...
//__GROUND__
  ...
  vec3 outsideView(vec2 vp, float ax, float yaw, float pitch) {
    ...
    for (int L = 0; L < 2; L++) {
      ...
      float rim = smoothstep(0.50, 0.62, cl) * smoothstep(0.70, 0.58, cl);
      col += mix(uSunGlow, vec3(1.0), 0.3) * rim * (0.18 + 0.5 * westWarm) * smoothstep(0.42, 1.0, vp.y) * (0.6 - fl * 0.2);
    ...
    // 評価でミルク色の靄が強すぎ低コントラストと指摘→ 0.24→0.16 に薄め中景の階調を残す（美術-M1）。
    float haze = smoothstep(0.54, 0.40, vp.y) * smoothstep(0.30, 0.46, vp.y);
    col = mix(col, mix(uHorizon, uSkyMid, 0.4), haze * 0.16);
  ...
  void main() {
    ...
    float xb = 0.185 / max(asp, 1.0);
    float winL = xb, winR = 1.0 - xb, winB = 0.175, winT = 0.845;
    // 開口（角を少し丸める）
